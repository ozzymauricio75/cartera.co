...
if(is_array($recaudos_array) && !empty($recaudos_array)){
    ...
    foreach($recaudos_array as $drKey){    
        ...
        $cuota_capital = number_format($drKey['capital_cuota_recaudo'], 2, '.', ',');
        //$cuota_interes = $db->escape($drKey['interes_cuota_recaudo']);
        $cuota_mora = number_format($drKey['valor_mora_cuota_recaudo'], 2, '.', ',');      
        //$cuota_sobrecargo = $db->escape($drKey['sobrecargo_cuota_recaudo']);
        $cuota_valor_cuota = number_format($drKey['total_cuota_plan_pago'], 2, '.', ',');
        $cuota_valor_recaudado = number_format($drKey['total_valor_recaudado_estacuota'], 2, '.', ',');
        $cuota_valor_faltante = number_format($drKey['valor_faltante_cuota'], 2, '.', ',');
        $cuota_valor_recaulculado = number_format($drKey['valor_cuota_recaulcaldo_recaudos'], 2, '.', ',');
