...
function queryRecaudosRutaHoy($idRecaudo){
    ...
    $queryTbl = $db->get ("recaudos_tbl", 1, "id_recaudo, id_status_recaudo, numero_cuota_recaudos, total_cuota_plan_pago, total_valor_recaudado_estacuota, activa_mora, valor_mora_cuota_recaudo, valor_faltante_cuota, valor_cuota_recaulcaldo_recaudos, fecha_recaudo_realizado");
...
function queryEspecificaRutasHoy($idRuta_, $idSSUser_){
    global $db;
    global $dateFormatDB;
    $dataQuery = array();

    $db->where('id_ruta', $idRuta_);
    $db->where('id_cobrador', $idSSUser_);
...
function queryRutasDetalleHoy($dateFormatDB, $idSSUser){
    ...
    if ($rowQueryTbl > 0){
        ...
        foreach ($queryTbl as $qKey) {
            ...
            $dataEspecificaRuta = queryEspecificaRutasHoy($idRuta, $idSSUser);
...
$totalDineroRecaudar = 0;
$activaMoraRecaudoHoy = 0;
            ...
            foreach($datasEspecificaRutaHoy as $derKey){
                ...
                if(is_array($datasRecaudosHoy) && !empty($datasRecaudosHoy)){
                    foreach($datasRecaudosHoy as $drhKey){
                        ...
                        $activaMoraRecaudoHoy = $datasRecaudosHoy['activa_mora'];
                        $valorMoraRecaudoHoy = $datasRecaudosHoy['valor_mora_cuota_recaudo'];


                    }
                }
                if($activaMoraRecaudoHoy==""){
                  $activaMoraRecaudoHoy = 0;
                }

                $cuotaConMora = ($activaMoraRecaudoHoy == 1) ? $valorMoraRecaudoHoy : 0;
                //CALCULO CUOTA
                $totalDineroRecaudar = $totalDineroRecaudar + $valorCuotaRecaudoHoy + $cuotaConMora;
                $totalDineroRutaHoy = $totalDineroRutaHoy + ($valorCuotaRecaudoHoy + $cuotaConMora - $valorRecaudadoRecaudoHoy);
...
if($numeRecaudosRecibidosHoy > 0){
    ...
    $progresoRecaudos = round(($recaudosRecibidosHoy/$totalDineroRecaudar)*100, 1, PHP_ROUND_HALF_DOWN);
}
//echo $recaudosRecibidosHoy/$totalDineroRecaudar;
...
$db->where("id_cobrador", $idSSUser);
$db->where("fecha_dinero_recibido", $dateFormatDB);
$qrecibido = $db->getOne("dinero_recibido_tbl", "SUM(total_valor_recibido) AS totaldinerorecibido");
if(is_array($qrecibido) && !empty($qrecibido)){
    $totalDineroRecibidoHoy = $qrecibido["totaldinerorecibido"];
            ...
            <div class="row ">
                <div class="col-xs-12 margin-bottom-md">
                    <div class="box box25 box-solid bg-green-gradient">
                        ...
                        <div class="box-body">
                            <p class="text-size-x6 text-center">
                            <span class="margin-right-xs text-size-x7">$</span><?php echo $totalDineroRecibidoHoyFormat; //$recaudosRecibidosHoyFormat; ?>
                            </p>
                        </div>
                    </div>
                </div>
