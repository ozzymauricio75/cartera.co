...
/*
TOTAL GASTOS
 ' muestra el valor gastado acumulado desde que se inicio actividad hasta el dia actual
*/
$gastos_acumulado = 0;
$gastos_acumulado = queryGastos();
$total_gastos = number_format($gastos_acumulado, 0,",",".");  

/*
*TOTAL DINERO RECAUDADO
*/
$total_dinero_recaudado = queryDineroRecaudado();
$totalDineroRecaudado = number_format($total_dinero_recaudado, 0,",","."); 
    ...
    if($actividad_caja_menor == "cuadre"){
        ...
        */
        ...
        if($fechaValorDisponible_base != "0000-00-00"){
            ...
            $credito->where("id_acreedor",$idAcreedor);
            ...
            $db->where("fecha_inicio_plan_pago",$fechaValorDisponible_base, ">=");
            $queryPrestadoActual = $db->getOne ("planes_pago_tbl pp", "SUM(pp.valor_credito_plan_pago) AS valorprestadoactual");
            if(count($queryPrestadoActual)>0){
                $valor_prestado_actual=$queryPrestadoActual['valorprestadoactual'];
            }*/
...
$totalDisponiblePrestar = $define_user_capital_inicial - $dineroPrestadoArr - $gastos_acumulado + $total_dinero_recaudado + $capitalInyectado;
...
function queryValorRecaudosRutaHoy($idAcreedor_, $fechaRecaudo_){
    ...
    $credito->where('id_status_credito', array(1,3), "IN");    
    $credito->where('id_acreedor', $idAcreedor_);
    $credito->get('creditos_tbl', null, 'code_consecutivo_credito');
    
    $db->join($credito, 'cre.code_consecutivo_credito = rec.ref_recaudo');     
    $db->where('rec.fecha_max_recaudo', $fechaRecaudo_, "<=");    
    $db->where('rec.id_status_recaudo', '1', "!="); //ESTADO QUE INDICA QUE LA CUOTA AUN NO SE HA PAGADO     
    $queryTbl = $db->get ("recaudos_tbl rec", null, "rec.ref_recaudo, rec.capital_cuota_recaudo, rec.total_cuota_plan_pago, rec.total_valor_recaudado_estacuota, rec.activa_mora, rec.valor_mora_cuota_recaudo");
            ...
            <div class="modal-content">
                ...
                <div class="modal-body grey lighten-4">
                    ...
                    <form id="cuadreform" method="post">
                        ...
                        <input id="totalcobrarhoyinput" name="totalcobrarhoyinput" type="hidden" value="<?php echo $totalDineroRutaHoy; ?>">
                        <input id="totalabonocapitalhoyinput" name="totalabonocapitalhoyinput" type="hidden" value="<?php echo $total_capital_ruta_hoy; ?>">
                        <input id="totalutilidadhoyinput" name="totalutilidadhoyinput" type="hidden" value="<?php echo $total_utilidad_ruta_hoy; ?>">
                        <input id="totalingresosinput" name="totalingresosinput" type="hidden">
...
<script type="text/javascript">
    ...
    $("#guardacuadre").click(function(){
        ...
        var valcobrarhoy =  $("#totalcobrarhoyinput").val();
        var abonocapitalhoy =  $("#totalabonocapitalhoyinput").val();
        var utilidadhoy =  $("#totalutilidadhoyinput").val();          
        ...
        */
        ...
        formData.append("valcobrarhoypost", valcobrarhoy);
        formData.append("abonocapitalhoypost", abonocapitalhoy);
        formData.append("utilidadhoypost", utilidadhoy);         
