...
function queryDineroRecibido($refrecaudo_, $numeroCuota_){
    global $db;
    $dataQuery = array();
    $valorRecaudado = 0;

    $db->where('consecutivo_credito', $refrecaudo_);
    $db->where('numero_cuota_recaudos', $numeroCuota_);
    $queryTbl = $db->getOne ("dinero_recibido_tbl", "total_valor_recibido");

    $rowQueryTbl = count($queryTbl);
    if ($rowQueryTbl > 0){
        $valorRecaudado = $queryTbl['total_valor_recibido'];
    }
...
if(is_array($datas_array) && !empty($datas_array)){
    foreach($datas_array as $daKey){
        ...
        $activamora = isset($daKey['activamoracuota'])? $db->escape($daKey['activamoracuota']) : "";
        $valormora = isset($daKey['valormoracuota'])? $db->escape($daKey['valormoracuota']) : "";
        $valorbasecuota = isset($daKey['valorcuota'])? $db->escape($daKey['valorcuota']) : "";
        $valorrecaudadocuota = isset($daKey['valorrecaudadocuota'])? $db->escape($daKey['valorrecaudadocuota']) : "";
        $fechacobrar = ($daKey['fechacobrar'] == "0000-00-00")? "" : $db->escape($daKey['fechacobrar']);
        ...
        //formato datos
        $fecha_cobrar_human = ($fechacobrar != "") ? date("d/m/Y",strtotime($fechacobrar)) : "";
        $fecha_recaudo_human = ($fecharecaudo != "") ? date("d/m/Y",strtotime($fecharecaudo)) : "";
        ...
        *VALORES DATAS JSON
        */
        //"dinerorecibido":"'.$dinero_recibido_cuota.'", $valorrecaudadocuota
        $layoutDataItem.='{
        "item":"'.$num.'",
