...
*QUERY CREDITO
*/
function queryCreditoParametro($idSSUser_, $qUser_){
    global $db;  
    $dataQuery = array();
    $datasPlanPago = array();
    $datasRecaudos = array();
    ...
    $queryTbl = array();
    if($id_user != ""){
        $db->where('id_deudor', $id_user);   
        $db->where('id_acreedor', $idSSUser_);        
        $queryTbl = $db->get("creditos_tbl", null, "id_deudor, id_creditos, id_status_credito, code_consecutivo_credito");        
    }else if($ref_credito != ""){
        $db->where('code_consecutivo_credito', $ref_credito);      
        $db->where('id_acreedor', $idSSUser_);        
        $queryTbl = $db->get("creditos_tbl", null, "id_deudor, id_creditos, id_status_credito, code_consecutivo_credito");        
    }    
    ...
    if ($rowQueryTbl > 0){            
        foreach ($queryTbl as $qKey) {                        
            $statusCredito = queryStatusdeCredito($qKey["id_status_credito"]);
...
function queryCredito($idSSUser_){
    global $db;  
    $dataQuery = array();
    $datasPlanPago = array();
    $datasRecaudos = array();
...
$datas_array = array();
if($qUser != ""){
    $datas_array = queryCreditoParametro($idSSUser, $qUser);
}else{
    $datas_array = queryCredito($idSSUser);
}
...
if(is_array($datas_array) && !empty($datas_array)){
    foreach($datas_array as $daKey){
        ...
        $statuscredito = $daKey['statuscredito'];
        //datos deudor
        ...
        if(is_array($datasplanepago) && !empty($datasplanepago)){
            ...
            //valor_credito_plan_pago, valor_pagar_credito, periocidad_plan_pago, numero_cuotas_plan_pago, plazocredito_plan_pago, fecha_inicio_plan_pago, fecha_fin_plan_pago, valor_cuota_plan_pago
                $valor_credito = isset($datasplanepago['valor_credito_plan_pago'])? $db->escape($datasplanepago['valor_credito_plan_pago']) : "";
                $valor_a_pagar = isset($datasplanepago['valor_pagar_credito'])? $db->escape($datasplanepago['valor_pagar_credito']) : "";
                $periciodad_credito = isset($datasplanepago['periocidad_plan_pago'])? $db->escape($datasplanepago['periocidad_plan_pago']) : "";
                $numero_cuotas_credito = isset($datasplanepago['numero_cuotas_plan_pago'])? $db->escape($datasplanepago['numero_cuotas_plan_pago']) : "";
                $plazo_credito = isset($datasplanepago['plazocredito_plan_pago'])? $db->escape($datasplanepago['plazocredito_plan_pago']) : "";
                $fecha_inicio_credito = isset($datasplanepago['fecha_inicio_plan_pago'])? $db->escape($datasplanepago['fecha_inicio_plan_pago']) : "";
                $fecha_fin_credito = ($datasplanepago['fecha_fin_plan_pago'] == "0000-00-00")? "-" : $db->escape($datasplanepago['fecha_fin_plan_pago']);
                $valor_cuota_credito = isset($datasplanepago['valor_cuota_plan_pago'])? $db->escape($datasplanepago['valor_cuota_plan_pago']) : "";
        ...
        if(is_array($datasrecaudos) && !empty($datasrecaudos)){
            foreach($datasrecaudos as $dcKey){              
                ...
                */
                ...
                "statuscredito":"'.$statuscredito.'",
                "valorprestado":"'.$valor_credito.'", 
                "valorpagarcredito":"'.$valor_a_pagar.'", 
                "periocidadcredito":"'.$periciodad_credito.'",
                "totalcuotascredito":"'.$numero_cuotas_credito.'",
                "plazocredito":"'.$plazo_credito.'",
                "fechainiciocredito":"'.$fecha_inicio_credito.'",
                "fechafincredito":"'.$fecha_fin_credito.'",
                "valorcuotacredito":"'.$valor_cuota_credito.'",
                "cuota":"'.$numero_cuota.'",
                "fechacobro":"'.$fecha_cobrar_human.'",
                "valorbasecuota":"'.$valor_cuota.'",  
                "valormoracuota":"'.$valor_mora_cuota.'",  
                "valorpagarcuota":"'.$valor_final_cuota.'",
