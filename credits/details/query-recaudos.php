...
if(is_array($recaudos_array) && !empty($recaudos_array)){
    ...
    foreach($recaudos_array as $drKey){    
        ...
        //DEFINE EL VALOR CUOTA FALTANTE
        $valor_faltante_cuota = 0;
        if($cuota_activamora == 1){
            $valor_faltante_cuota = $cuota_valor_cuota_base - $cuota_valor_recaudado_base + $cuota_mora_base;
        }else{
            $valor_faltante_cuota = $cuota_valor_cuota_base - $cuota_valor_recaudado_base;
        }
        
        $valor_faltante_cuota_format = number_format($valor_faltante_cuota, 0, ',', '.');
        ...
        */
        ...
        "cuotamora":"'.$valor_mora_esta_cuota.'",
        "cuotafinal":"'.$valor_cuota_final.'",
        "cuotarecaudado":"'.$cuota_valor_recaudado.'",
        "cuotafaltante":"'.$valor_faltante_cuota_format.'",
        "cuotafecha":"'.$cuota_fecha_recaudo.'",        
