...
if(isset($fieldPost) && $fieldPost == "additem"){
    ...
    *IDS USUARIOS
    */
    $idCreditoActual = (empty($_POST['creditactualpost']))? "" : $_POST['creditactualpost'];
        ...
        if ($ultimoCredito){
            ...
            if($newPlanPagos){
                ...
                if(is_array($errInsertDatasRecaudos) && count($errInsertDatasRecaudos) > 0){
                    ...
                    //cambiamos el estado del credito actual
                    $datasErr = "";
                    $datasUpdate = array(
                        'id_status_credito' => "7",
                        'fecha_cierre_definitivo_credito' => $dateFormatDB
                    );
                    $datasUpdateRecaudo = array(
                        'id_status_recaudo' => "7"
                    );

                    //actualizar campo en base de datos
                    $db->where('id_creditos', $idCreditoActual);
                    $goEdit = $db->update('creditos_tbl', $datasUpdate);

                    $db->where('id_creditos', $idCreditoActual);
                    $queryTbl = $db->getOne ("creditos_tbl", "code_consecutivo_credito");

                    $referenciaCredito = $queryTbl['code_consecutivo_credito'];
                    $db->where('ref_recaudo', $referenciaCredito);

                    $goEdi  = $db->update('recaudos_tbl', $datasUpdateRecaudo);
                    ...
                    if($goEdit === true){
                        ...
                        $errQueryTmpl .="<li class='list-group-item list-group-item-danger'><b>*** Algo salio mal ***</b><br>
                            <br>Wrong: <b>El nuevo credito fue creado exitosamente, pero el credito actual no logro cambiar su estado a Refinanciado</b>
                            <br>Erro: ".$datasErr."
