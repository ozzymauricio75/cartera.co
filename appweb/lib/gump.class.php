...
class GUMP
{
        ...
        foreach ($this->errors as $e) {
            ...
            switch ($e['rule']) {
                ...
                case 'validate_exact_len':
                    ...
                    break;  
                case 'validate_user':
                    $resp[] = "The <span class=\"$field_class\">$field</span> field may only contain alpha-numeric characters, and characters(_ - .)";
                    break;  
                case 'validate_pass':
                    $resp[] = "The <span class=\"$field_class\">$field</span> field may only contain alpha-numeric characters, and characters(!@#$%&()+.-_)";
                    break;
        ...
        foreach ($this->errors as $e) {
            ...
            switch ($e['rule']) {
                ...
                case 'validate_exact_len':
                    ...
                    break;
                case 'validate_user':
                    $resp[$field] = "The $field field may only contain alpha-numeric characters, and characters(_ - .)";
                    break;
                case 'validate_pass':
                    $resp[$field] = "The $field field may only contain alpha-numeric characters, and characters(!@#$%&()+.-_)"; 
                    break;
    ...
    */
    ...
    /**
     * Determine if the provided value contains only user characters.
     *
     * Usage: '<index>' => 'user'
     *
     * @param string $field
     ...
     * @return mixed
     */
    protected function validate_user($field, $input, $param = null)
    {
        if (!isset($input[$field]) || empty($input[$field])) {
            return;
        }

        if (!preg_match('/^([a-zA-Z0-9.-_])+$/i', $input[$field]) !== false) {
            return array(
                'field' => $field,
                'value' => $input[$field],
                'rule' => __FUNCTION__,
    ...
    /**
     * Determine if the provided value contains only password characters.
     *
     * Usage: '<index>' => 'pass'
     *
     * @param string $field
     ...
     * @return mixed
     */
    protected function validate_pass($field, $input, $param = null)
    {
        if (!isset($input[$field]) || empty($input[$field])) {
            return;
        }
    ...
    protected function validate_alpha_space($field, $input, $param = null)
    {
        ...
        if (!preg_match("/^([a-zA-Z0-9ÀÁÂÃÄÅÇÈÉÊËÌÍÎÏÒÓÔÕÖßÙÚÛÜÝàáâãäåçèéêëìíîïðòóôõöùúûüýÿñÑ(){}<>\/\\,.;:?¿@€&!¡*=-_#%\s\$\w\r\n\t\v\b\+\-\"\'])+$/i", $input[$field]) !== false) {
        //if (!preg_match("/^([a-z0-9ÀÁÂÃÄÅÇÈÉÊËÌÍÎÏÒÓÔÕÖßÙÚÛÜÝàáâãäåçèéêëìíîïðòóôõöùúûüýÿ(){}[],.;:?¿@€&!¡-+=_#%\s\$\w\r\n\t\v\b])+$/i", $input[$field]) !== false) {
            return array(
                'field' => $field,
                'value' => $input[$field],
                'rule' => __FUNCTION__,
    ...
    protected function validate_street_address($field, $input, $param = null)
    {
        ...
        $hasLetter = preg_match('/^[a-zA-Z1-9À-ÖØ-öø-ÿ]+\.?(( |\-)[a-zA-Z1-9À-ÖØ-öø-ÿ]+\.?)* (((#|[nN][oO]\.?) ?)?\d{1,4}(( ?[a-zA-Z0-9\-]+)+)?)$/D', $input[$field]); ///[a-zA-Z]/
    ...
    protected function validate_date($field, $input, $param = null)
    {
        ...
        $cdate3 = date('H:i', strtotime($input[$field]));

        if ($cdate1 != $input[$field] && $cdate2 != $input[$field] && $cdate3 != $input[$field] ) {
            return array(
                'field' => $field,
                'value' => $input[$field],
                'rule' => __FUNCTION__,
    ...
    protected function validate_phone_number($field, $input, $param = null)
    {
        ...
        $regex = '/^([\+]?[\d]{1,4} ?)?([\(]([\d]{1,3})[)] ?)?[0-9][0-9 \-]{6,}( ?([xX]|([eE]xt[\.]?)) ?([\d]{1,5}))?$/i';
