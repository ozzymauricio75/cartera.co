...
if(isset($fieldPost) && $fieldPost == "additem"){    
    ...
    */                         
    ...
    $filters = array(
        //'idcodeudor'=> 'trim|sanitize_string',
	...
	if($validated === TRUE && $checkTels !="error"){
        ...
        //*************
        */
        $db->where('email_codeudor', $Email);
        $db->where('email_codeudor', "", "!=");
        $emailExist = $db->getOne('codeudor_tbl');
        ...
        //***********
        $tablaExiQ = "codeudor_tbl";
        $campoExiQ = "cedula_codeudor";
        ...
        if($deudorExiste == false){//SI EXISTE EL REGISTRO
            ...
            //***********
            //PROCESO NUEVO REGISTRO (CODEUDOR)
            //***********
            ...
            foreach($nuevoPost as $valInsert){
                $dataInsert = array(        
                    ...
                    'primer_nombre_codeudor' => $nuevoPost['primernombre'],
                    'segundo_nombre_codeudor' =>$nuevoPost['segundonombre'],
                    'primer_apellido_codeudor'=>  $nuevoPost['PrimerApellido'],    
                    'segundo_apellido_codeudor'=> $nuevoPost['segundoapellido'],
                    'cedula_codeudor' =>$nuevoPost['NumeroDocumento'],
                    'fecha_nacimiento_codeudor'=> $nuevoPost['Fecha_Nacimiento'],  
                    'lugar_naciminto_codeudor'=> $nuevoPost['Lugar_Nacimiento'], 
                    'genero_codeudor'=> $nuevoPost['Genero'],  
                    'estado_civil_codeudor'=> $nuevoPost['Estado_Civil'],  
                    'email_codeudor' =>  $nuevoPost['Email'],
                    'tel_uno_codeudor' => $nuevoPost['Telefono1'],
                    'tel_dos_codeudor' =>$nuevoPost['Telefono2'],
                    'dir_domicilio_codeudor'=>  $nuevoPost['Direccion_Residencia'],
                    'complemento_dir_codeudor'=> $nuevoPost['Complemento'],
                    'barrio_domicilio_codeudor' =>$nuevoPost['Barrio'],
                    'tipo_vivienda_codeudor'=> $nuevoPost['Tipo_Vivienda'],       
                    'estrato_social_codeudor'=> $nuevoPost['Estrato'],            
                    //'dir_geo_codeudor' => $direccionFullPost,
                    //'latitud_geo_codeudor' => $latstore,
                    //'longitud_geo_codeudor'=>$lngstore,
                    //'url_maps_codeudor'=> $idMapPost,
                    //'codigo_geo_ciudad_codeudor' => $usercityUser,
                    //'codigo_geo_estado_codeudor' => $userstate,
                    //'codigo_geo_pais_codeudor'=> $paisPost,
                    //'codigo_postal_geo_codeudor' =>$userzip,                    
                    'nivel_escolaridad_codeudor' => $nuevoPost['escolaridad'],    
                    'oficio_codeudor' => $nuevoPost['Oficio'],
                    'profesion_codeudor' =>$nuevoPost['Profesion'],
                    'nombre_empresa_codeudor'=>$nuevoPost['Nombre_Empresa'],
                    'cargo_empresa_codeudor'=>$nuevoPost['Cargo'],
                    'tel_empresa_codeudor'=>$nuevoPost['Telefono'],
                    'dir_empresa_codeudor'=>$nuevoPost['Direccion'],
                    'ciudad_empresa_codeudor'=>$nuevoPost['Ciudad_Empresa'],      
                    'comentarios_codeudor'=>$nuevoPost['comentarios'],
                    'fecha_alta_codeudor'=> $dateFormatDB
                );  
            }
            //echo "<pre>";
            ...
            //if (!$id){
            //$db->where ('id_codeudor',$id_ref_deudor); 
            $newCodeudorINS = $db->insert ('codeudor_tbl', $dataInsert);
            if ($newCodeudorINS){  
                            ...
                            if(upimgfile($valF['name'], $valF['tmp_name'])){
                                ...
                                if(redimensionaImgFile($valF['name'], $valF['type'], $nameFileSponsorF, $patFileEnd, $squareFile)){ 
                                    ...
                                    if(redimensionaImgFile($valF['name'], $valF['type'], $nameFileSponsorF, $patFileEndThumb, $squareFileThumb)){
                                        ...
                                        $idSponsor = $db->rawQuery("INSERT INTO documentos_requisitos_tbl (id_acreedor, id_codeudor, referencia_documento_requisito, file_documento_requisito) VALUES('".$id_acreedor."', '".$newCodeudorINS."','".$refAlbumInsert."',  '".$nameFileSponsorF.".jpg')");
                                        if(!isset($idSponsor)){ 
                ...
                $dataUpdateCredito = array("id_codeudor"=>$newCodeudorINS);
                ...
                //$response['error'] = "Error al insertar el Codeudor: ".$db->getLastQuery() ."\n". $db->getLastError();
                ...
                $errQueryTmpl_ins .="<li class='list-group-item list-group-item-danger'><b>*** Algo salio mal ***</b><br>
                    <br>Wrong: <b>No se pudo crear este Codeudor</b>
            ...
            foreach($valRules as $key => $v){
                ...
                switch($errFiel){
                    /*case 'idcodeudor' :
                        ...
                        <br>".$usertyped."
                        <br>Parece que ha ocurrido un error en el momento de crear este Codeudor, por favor, intenta crear un nuevo Credito, en la opción CREAR / NUEVO CREDITO</li>";
                    break;  */                      
                    case 'primernombre' :
...
/*
*ADICIONAR CODEUDOR EXISTENTE A CREIDTO
*/
