...
if(isset($fieldPost) && $fieldPost == "additem"){    
    ...
    */
    ...
    //$id_reffami = (empty($_POST['codeitemform']))? "" : $_POST['codeitemform'];
    ...
    */                         
    ...
    $filters = array(
        //'idreffami'=> 'trim|sanitize_string',
	...
	if($validated === TRUE && $checkTels != "error"){
        ...
        //*************
        */
        $db->where('email_referencia_familiar', $Email);
        $db->where('email_referencia_familiar', "", "!=");
        $emailExist = $db->getOne('referencia_familiar_tbl');
        ...
        //***********
        $tablaExiQ = "referencia_familiar_tbl";
        $campoExiQ = "cedula_referencia_familiar";
        ...
        if($deudorExiste == false){//SI EXISTE EL REGISTRO
            ...
            foreach($nuevoPost as $valInsert){
                $dataInsert = array(  
                    ...
                    'primer_nombre_referencia_familiar' => $nuevoPost['primernombre'],
                    'segundo_nombre_referencia_familiar' =>$nuevoPost['segundonombre'],
                    'primer_apellido_referencia_familiar'=>  $nuevoPost['PrimerApellido'],   
                    'segundo_apellido_referencia_familiar'=> $nuevoPost['segundoapellido'],
                    'cedula_referencia_familiar' =>$nuevoPost['NumeroDocumento'],
                    'fecha_nacimiento_referencia_familiar'=> $nuevoPost['Fecha_Nacimiento'], 
                    'lugar_naciminto_referencia_familiar'=> $nuevoPost['Lugar_Nacimiento'], 
                    'genero_referencia_familiar'=> $nuevoPost['Genero'],  
                    'estado_civil_referencia_familiar'=> $nuevoPost['Estado_Civil'],  
                    'email_referencia_familiar' =>  $nuevoPost['Email'],
                    'tel_uno_referencia_familiar' => $nuevoPost['Telefono1'],
                    'tel_dos_referencia_familiar' =>$nuevoPost['Telefono2'],
                    'dir_domicilio_referencia_familiar'=>  $nuevoPost['Direccion_Residencia'],
                    'complemento_dir_referencia_familiar'=> $nuevoPost['Complemento'],
                    'barrio_domicilio_referencia_familiar' =>$nuevoPost['Barrio'],
                    'tipo_vivienda_referencia_familiar'=> $nuevoPost['Tipo_Vivienda'],       
                    'estrato_social_referencia_familiar'=> $nuevoPost['Estrato'],           
                    //'dir_geo_referencia_familiar' => $direccionFullPost,
                    //'latitud_geo_referencia_familiar' => $latstore,
                    //'longitud_geo_referencia_familiar'=>$lngstore,
                    //'url_maps_referencia_familiar'=> $idMapPost,
                    //'codigo_geo_ciudad_referencia_familiar' => $usercityUser,
                    //'codigo_geo_estado_referencia_familiar' => $userstate,
                    //'codigo_geo_pais_referencia_familiar'=> $paisPost,
                    //'codigo_postal_geo_referencia_familiar' =>$userzip,                    
                    'nivel_escolaridad_referencia_familiar' => $nuevoPost['escolaridad'],   
                    'oficio_referencia_familiar' => $nuevoPost['Oficio'],
                    'profesion_referencia_familiar' =>$nuevoPost['Profesion'],
                    'nombre_empresa_referencia_familiar'=>$nuevoPost['Nombre_Empresa'],
                    'cargo_empresa_referencia_familiar'=>$nuevoPost['Cargo'],
                    'tel_empresa_referencia_familiar'=>$nuevoPost['Telefono'],
                    'dir_empresa_referencia_familiar'=>$nuevoPost['Direccion'],
                    'ciudad_empresa_referencia_familiar'=>$nuevoPost['Ciudad_Empresa'], 
                    'comentarios_referencia_familiar'=>$nuevoPost['comentarios'],
                    'fecha_alta_referencia_familiar'=> $dateFormatDB
                );  
            }
            //echo "<pre>";
            ...
            //if (!$id){
            //$db->where ('id_referencia_familiar',$id_reffami); 
            $newRefFamiINS = $db->insert ('referencia_familiar_tbl', $dataInsert);
            if ($newRefFamiINS){  
                            ...
                            if(upimgfile($valF['name'], $valF['tmp_name'])){
                                ...
                                if(redimensionaImgFile($valF['name'], $valF['type'], $nameFileSponsorF, $patFileEnd, $squareFile)){ 
                                    ...
                                    if(redimensionaImgFile($valF['name'], $valF['type'], $nameFileSponsorF, $patFileEndThumb, $squareFileThumb)){
                                        ...
                                        $idSponsor = $db->rawQuery("INSERT INTO documentos_requisitos_tbl (id_acreedor, id_referencia_familiar, referencia_documento_requisito, file_documento_requisito) VALUES('".$id_acreedor."', '".$newRefFamiINS."','".$refAlbumInsert."',  '".$nameFileSponsorF.".jpg')");
                                        if(!isset($idSponsor)){ 
                ...
                $dataUpdateCredito = array("id_referencia_familiar"=>$newRefFamiINS);
                ...
                //$response['error'] = "Error al insertar la referencia familiar: ".$db->getLastQuery() ."\n". $db->getLastError();
                $errInsertDatas = $db->getLastErrno();

                $errQueryTmpl_ins ="<ul class='list-group text-left'>";
                $errQueryTmpl_ins .="<li class='list-group-item list-group-item-danger'><b>*** Algo salio mal ***</b><br>
                    <br>Wrong: <b>No se pudo crear esta referencia familiar</b>
                    <br>Erro: ".$errInsertDatas."
                    <br>Puedes intentar de nuevo
                    <br>Si el error continua, por favor entre en contacto con soporte</li>";
                $errQueryTmpl_ins .="</ul>";
