...
function queryValorDisponible($idAcreedor_){
    ...
    $queryTbl = $db->get ("caja_menor_tbl", 1, "valor_disponible_caja_menor, fecha_cuadre_caja_menor");
...
function queryCapitalInyectado($idAcreedor_){
    global $db;  
    $dataQuery = 0;//array();
    
    $db->where('id_acreedor', $idAcreedor_);
    $db->orderBy('id_inyeccion_capital', 'desc');
    $queryTbl = $db->getOne ("inyeccion_capital_tbl", "valor_inyeccion_capital");
    
    if(count($queryTbl)>0){
        $dataQuery = $queryTbl["valor_inyeccion_capital"];
    }
...
function queryStatusCuadre($idAcreedor_, $fechaCuadre_){
    ...
    $queryTbl = $db->get ("caja_menor_tbl", 1, "fecha_cuadre_caja_menor, status_caja_menor");
