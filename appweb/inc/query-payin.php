...
//QUERY DEUDOR ORDENADO POR NOMBRE
function queryDeudor_FULL_Ordenado($idDeudor_){
    global $db;
    $dataQuery = array();
...
function queryValorAcumulado($concecutivo){
    ...
    //$actiMoraRecaudos = $queryTbl['actimora'];
    //$valorMoralRecaudos = $queryTbl['valormora'];
...
/*//--VALOR MORA ACUMULADO POR CREDITO--//*/
function queryValorMoraAcumulado($concecutivo){
    global $db;
    global $dateFormatDB;
    $dataQuery = array();
    ...
    $db->where('id_status_recaudo', '1', '!='); //ESTADO QUE INDICA QUE LA CUOTA AUN NO SE HA PAGADO
    $db->where('fecha_max_recaudo', $dateFormatDB, '<=');
    $queryTbl = $db->getOne ("recaudos_tbl", "SUM(valor_mora_cuota_recaudo) AS totalmora");

    $totalmora = $queryTbl['totalmora'];
    return $totalmora;
}
...
//SOBRE LA RUTA
function queryRecaudosRutaDetallesCobrador($idRuta_, $idSSUser_){
    global $db;
    global $dateFormatDB;
    $dataQuery = array();
    $datasCuota = array();
...
//SOBRE LA RUTA SI LA BUSQUEDA ES ESPECIFICA POR UN DEUDOR
function queryRecaudosRutaDetallesDeudor($idRuta_, $id_deudor_){
    global $db;
    global $dateFormatDB;
    $dataQuery = array();
    $datasCuota = array();
