...
//DETALLES CREDITOS DEL USUARIO DEUDOR x ACREEDOR PARA ABONOS
function queryDeudorCreditosxAcreedor($idDeudor_,$idAcreedor_){
    global $db;
    $dataQuery = array();
...
//DETALLES CREDITOS DEL USUARIO DEUDOR x ACREEDOR PARA ABONOS
function queryDeudorCreditosxAcreedorEstado($idDeudor_,$idAcreedor_){
    global $db;
    $dataQuery = array();
...
//DETALLES CREDITOS DEL USUARIO DEUDOR x ACREEDOR PARA ABONOS
function queryDeudorEstadoCredito($idDeudor_,$idAcreedor_){
  global $db;
  $dataQuery = array();
  $datasCreditos = array();
...
//    $db->where('id_status_perfil_deduor', "5", "!=");
  ...
  if ($rowQueryTbl > 0){
      foreach ($queryTbl as $qKey) {
          ...
          $datasCreditos = queryDeudorCreditosxAcreedorEstado($qKey['id_deudor'],$idAcreedor_);
...
//DETALLES CREDITOS DEL USUARIO DEUDOR x ACREEDOR
function queryDeudorCreditosAcreedor($idDeudor_,$idAcreedor_){
    global $db;
    $dataQuery = array();
...
//DETALLES DEL USUARIO DEUDOR SEGUN ACREEDOR
function queryDeudorDetallesAcreedor($idDeudor_,$idAcreedor_){
    global $db;
    $dataQuery = array();
    $datasCreditos = array();
    $datasDocumentos = array();
...
//    $db->where('id_status_perfil_deduor', "5", "!=");
    ...
    if ($rowQueryTbl > 0){
        foreach ($queryTbl as $qKey) {
            ...
            //CREDITOS DE ESTE DEUDOR
            //$id_item = $qKey['id_deudor'];
            $datasCreditos = queryDeudorCreditosxAcreedor($qKey['id_deudor'],$idAcreedor_);
