...
function queryCuotas($idAcreedor_, $idCobrador_, $fechaHoy_){
    ...
    $credito->where('id_cobrador', $idCobrador_);
    ...
    $db->where('id_status_recaudo', '1', '!=');
    $db->where('fecha_max_recaudo', $fechaHoy_, '<=');
    //$queryTbl = $db->get ("recaudos_tbl rec", null, "rec.id_acreedor, rec.id_recaudo, rec.ref_recaudo, rec.id_plan_pago, rec.id_cobrador, rec.activa_mora, rec.id_status_recaudo, rec.numero_cuota_recaudos, rec.capital_cuota_recaudo, rec.interes_cuota_recaudo, rec.valor_mora_cuota_recaudo, rec.sobrecargo_cuota_recaudo, rec.total_cuota_plan_pago, rec.total_valor_recaudado_estacuota, rec.valor_faltante_cuota, rec.valor_cuota_recaulcaldo_recaudos, rec.fecha_max_recaudo, rec.fecha_recaudo_realizado, rec.comentarios_recaudo");
    $queryTbl = $db->get ("recaudos_tbl rec", null, "rec.id_recaudo, rec.id_plan_pago, rec.id_cobrador, rec.ref_recaudo");
...
function queryValorRecaudosRutaHoy($idAcreedor_, $fechaRecaudo_){
    ...
    $queryTbl = $db->get ("recaudos_tbl", null, "ref_recaudo, total_cuota_plan_pago, valor_faltante_cuota, valor_cuota_recaulcaldo_recaudos, total_valor_recaudado_estacuota, activa_mora, valor_mora_cuota_recaudo");
...
function queryRecaudosRuta($idRecaudo){
    ...
    $queryTbl = $db->get ("recaudos_tbl", 1, "id_recaudo, total_cuota_plan_pago, total_valor_recaudado_estacuota, activa_mora, valor_mora_cuota_recaudo");
...
function queryEspecificaRutas($idRuta_, $idCobrador_){
    global $db;
    global $dateFormatDB;
    $dataQuery = array();
...
//QUERY RUTAS
function queryRutas($idAcreedor_, $fechaHoy_, $idCobrador_){
    global $db;
    global $dateFormatDB;
    $dataQuery  = array();
    $idCobrador = $idCobrador_;
    ...
    if ($rowQueryTbl > 0){
        ...
        foreach ($queryTbl as $qKey) {
            ...
            $dataEspecificaRuta = queryEspecificaRutas($idRuta, $idCobradorRuta);
