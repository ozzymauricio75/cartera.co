...
/**DINERO RECIBIDO DEL DIA*/
function queryValorDineroRecibido($fechaRecaudo_, $idAcreedor_){
    global $db;
    $dataQuery = array();

    $db->where('id_acreedor', $idAcreedor_);
    $db->where('fecha_dinero_recibido', $fechaRecaudo_);
    $db->where('id_status_recibido', '3', '!=');//ESTADO QUE INDICA QUE LA CUOTA FUE PAGADA COMPLETA O ABONADA
    //$db->where('id_status_recaudo', '1');
    //$db->orWhere('id_status_recaudo', '2');
    $queryTbl = $db->getOne ("dinero_recibido_tbl", "SUM(total_valor_recibido) AS varlorrecibido");

    $valorrecibido = $queryTbl['varlorrecibido'];
...
if($fechaMiPrimerCredito != "0000-00-00"){
    function queryIngresosDiarios(){
        ...
        for ($midia = $fechainicio; $midia <= $fechafin; $midia += $diainc) {
            ...
            //VALOR DINERO RECIBIDO
            $arrayValorDineroRecibidoCuadre = queryValorDineroRecibido(date('Y-m-d', $midia), $idSSUser);
            ...
            //if(is_array($arrayRecaudosCuadrar) && !empty($arrayRecaudosCuadrar)){//-->**solo me mostraria los dias que tengan recaudos registrados
                $dataIngresosDiarios[] = array(
                    ...
                    'valortotalingresos' => empty($arrayValorDineroRecibidoCuadre)? 0 : $arrayValorDineroRecibidoCuadre,
