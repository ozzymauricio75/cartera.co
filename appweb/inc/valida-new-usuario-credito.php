...
if(isset($fieldPost) && $fieldPost == "additem"){    
    ...
    */
    ...
    $type_user = (empty($_POST['typeuserform']))? "" : $_POST['typeuserform'];
    ...
    */                         
    ...
    if($opciidentiform == "cedula"){
        ...
        $rules = array(
            ...
            'Fecha_Nacimiento'=>'date|min_age,18',
            ...
            'geodireccion'=> 'required|alpha_space',   
        );
	...
	if($validated === TRUE && $checkTels != "error" && $check_Cedula != "error" &&  $check_Nit != "error"){
        ...
        //*************
        */
        $db->where('email_deudor', $Email);
        $db->where('email_deudor', "", "!=");
        $emailExist = $db->getOne('deudor_tbl');
        ...
        if($deudorExiste == false){//SI EXISTE EL REGISTRO
            ...
            foreach($nuevoPost as $valInsert){
                $dataInsert = array(
                    ...
                    'primer_nombre_deudor' => $nuevoPost['primernombre'],
                    'segundo_nombre_deudor' =>$nuevoPost['segundonombre'],
                    'primer_apellido_deudor'=>  $nuevoPost['PrimerApellido'],    
                    'segundo_apellido_deudor'=> $nuevoPost['segundoapellido'],
                    'cedula_deudor' =>$nuevoPost['NumeroDocumento'],
                    'fecha_nacimiento_deudor'=> $nuevoPost['Fecha_Nacimiento'],  
                    'lugar_naciminto_deudor'=> $nuevoPost['Lugar_Nacimiento'], 
                    'genero_deudor'=> $nuevoPost['Genero'],  
                    'estado_civil_deudor'=> $nuevoPost['Estado_Civil'],  
                    'email_deudor' =>  $nuevoPost['Email'],
                    'tel_uno_deudor' => $nuevoPost['Telefono1'],
                    'tel_dos_deudor' =>$nuevoPost['Telefono2'],
                    'dir_domicilio_deudor'=>  $nuevoPost['Direccion_Residencia'],
                    //'complemento_dir_deudor'=> $nuevoPost['Complemento'],
                    'barrio_domicilio_deudor' =>$nuevoPost['Barrio'],
                    'tipo_vivienda_deudor'=> $nuevoPost['Tipo_Vivienda'],        
                    'estrato_social_deudor'=> $nuevoPost['Estrato'],             
                    ...
                    'nivel_escolaridad_deudor' => $nuevoPost['escolaridad'],     
                    'oficio_deudor' => $nuevoPost['Oficio'],
                    'profesion_deudor' =>$nuevoPost['Profesion'],
                    'nombre_empresa_deudor'=>$nuevoPost['Nombre_Empresa'],
                    'nit_referencia_comercial' =>$nuevoPost['nitempresa'],
                    'nombre_contato_referencia_comercial'=>$nuevoPost['Nombre_contacto'],
                    'cargo_empresa_deudor'=>$nuevoPost['Cargo'],
                    'tel_empresa_deudor'=>$nuevoPost['Telefono'],
                    'dir_empresa_deudor'=>$nuevoPost['Direccion'],
                    'ciudad_empresa_deudor'=>$nuevoPost['Ciudad_Empresa'],       
                    'comentarios_deudor'=>$nuevoPost['comentarios'],
                    'fecha_alta_deudor'=> $dateFormatDB
                );  
            }
            //echo "<pre>";
            ...
            $newUsuarioAsignadoINS = $db->insert ('deudor_tbl', $dataInsert);
            if ($newUsuarioAsignadoINS){  
                //***********
                //DATOS REF
                //***********
                            ...
                            if(upimgfile($valF['name'], $valF['tmp_name'])){
                                ...
                                if(redimensionaImgFile($valF['name'], $valF['type'], $nameFileSponsorF, $patFileEnd, $squareFile)){ 
                                    ...
                                    if(redimensionaImgFile($valF['name'], $valF['type'], $nameFileSponsorF, $patFileEndThumb, $squareFileThumb)){
                                        ...
                                        $idSponsor = $db->rawQuery("INSERT INTO documentos_requisitos_tbl (id_acreedor, id_deudor, referencia_documento_requisito, file_documento_requisito) VALUES('".$id_reffami."', '".$newUsuarioAsignadoINS."','".$refAlbumInsert."',  '".$nameFileSponsorF.".jpg')");
                                        if(!isset($idSponsor)){ 
                ...
                /*
                *ACTUALIZA CREDITO Y PLAN DE PAGOS CON LA NUEVA REFERENCIA / O USUARIO ASIGNADO
                */

                //DEFINIMOS QUE TIPO DE USUARIO ES
                $typo_usuario_asignado = "";
                switch($type_user){
                    case "deudor":
                        $typo_usuario_asignado = "Deudor";
                        
                        //ACTUALIZA CREDITO CON AL NUEVA REFRENCIA
                        $dataUpdateCredito = array("id_deudor"=>$newUsuarioAsignadoINS);
                        $db->where("id_creditos", $id_credito);
                        $actualizaCredito = $db->update("creditos_tbl", $dataUpdateCredito);


                        //ACTUALIZA PLANE PAGOS
                        $dataUpdatePlanPago = array("id_deudor"=>$newUsuarioAsignadoINS);
                        $db->where("id_credito", $id_credito);
                        $actualizaPlanPago = $db->update("planes_pago_tbl", $dataUpdatePlanPago);
                        
                    break;
                    case "codeudor":
                        $typo_usuario_asignado = "Codeudor";
                        
                        //ACTUALIZA CREDITO CON AL NUEVA REFRENCIA
                        $dataUpdateCredito = array("id_codeudor"=>$newUsuarioAsignadoINS);
                        $db->where("id_creditos", $id_credito);
                        $actualizaCredito = $db->update("creditos_tbl", $dataUpdateCredito);
                        
                    break;
                    case "ref_personal":
                        $typo_usuario_asignado = "Ref. Personal";
                        
                        //ACTUALIZA CREDITO CON AL NUEVA REFRENCIA
                        $dataUpdateCredito = array("id_referencia_personal"=>$newUsuarioAsignadoINS);
                        $db->where("id_creditos", $id_credito);
                        $actualizaCredito = $db->update("creditos_tbl", $dataUpdateCredito);
                        
                    break;
                    case "ref_familiar":
                        $typo_usuario_asignado = "Ref. Familiar";
                        
                        //ACTUALIZA CREDITO CON AL NUEVA REFRENCIA
                        $dataUpdateCredito = array("id_referencia_familiar"=>$newUsuarioAsignadoINS);
                        $db->where("id_creditos", $id_credito);
                        $actualizaCredito = $db->update("creditos_tbl", $dataUpdateCredito);
                    break;
                    case "ref_comercial":
                        $typo_usuario_asignado = "Ref. Comercial";
                        
                        //ACTUALIZA CREDITO CON AL NUEVA REFRENCIA
                        $dataUpdateCredito = array("id_referencia_comercial"=>$newUsuarioAsignadoINS);
                        $db->where("id_creditos", $id_credito);
                        $actualizaCredito = $db->update("creditos_tbl", $dataUpdateCredito);
                ...
                //CREAMOS EL ARRAY CON LOS DATOS DE USUARIO ASIGNADO
                $dataUserCredito = array(
                    "id_credito" => $id_credito,
                    "id_usuario_credito" => $newUsuarioAsignadoINS,
                    "tipo_usuario_asignado" =>$typo_usuario_asignado,
                    "tag_tipo_usuario"=>$type_user
                );
                
                $usuarioAsignado = $db->insert("usuario_asignado_credito", $dataUserCredito);
                ...
                if($usuarioAsignado){
                    ...
                    $errQueryTmpl_ins .="<li class='list-group-item list-group-item-danger'><b>*** Algo salio mal ***</b><br>
                        <br>Wrong: <b>No se pudo asignar este usuario al credito</b>
            ...
            foreach($valRules as $key => $v){
                ...
                switch($errFiel){
                    ...
                    case 'Fecha_Nacimiento' :
                        ...
                        <br>El usuario debe ser mayor de edad
...
if(isset($fieldPost) && $fieldPost == "addrefcredito"){    
    ...
    $id_ref_credito = (empty($_POST['itemusuario']))? "" : $_POST['itemusuario'];
    ...
    */
    ...
    switch($type_user){
        case "deudor":
            ...
            //ACTUALIZA CREDITO CON AL NUEVA REFRENCIA
            $dataUpdateCredito = array("id_deudor"=>$id_ref_credito);
            $db->where("id_creditos", $id_credito);
            $actualizaCredito = $db->update("creditos_tbl", $dataUpdateCredito);


            //ACTUALIZA PLANE PAGOS
            $dataUpdatePlanPago = array("id_deudor"=>$id_ref_credito);
            $db->where("id_credito", $id_credito);
            $actualizaPlanPago = $db->update("planes_pago_tbl", $dataUpdatePlanPago);
        ...
        case "codeudor":
            ...
            //ACTUALIZA CREDITO CON AL NUEVA REFRENCIA
            $dataUpdateCredito = array("id_codeudor"=>$id_ref_credito);
            $db->where("id_creditos", $id_credito);
            $actualizaCredito = $db->update("creditos_tbl", $dataUpdateCredito);
        ...
        case "ref_personal":
            ...
            //ACTUALIZA CREDITO CON AL NUEVA REFRENCIA
            $dataUpdateCredito = array("id_referencia_personal"=>$id_ref_credito);
            $db->where("id_creditos", $id_credito);
            $actualizaCredito = $db->update("creditos_tbl", $dataUpdateCredito);
        ...
        case "ref_familiar":
            ...
            //ACTUALIZA CREDITO CON AL NUEVA REFRENCIA
            $dataUpdateCredito = array("id_referencia_familiar"=>$id_ref_credito);
            $db->where("id_creditos", $id_credito);
            $actualizaCredito = $db->update("creditos_tbl", $dataUpdateCredito);
        break;
        case "ref_comercial":
    ...
    $dataUserCredito = array(
        "id_credito" => $id_credito,
        "id_usuario_credito" => $id_ref_credito,
        "tipo_usuario_asignado" =>$typo_usuario_asignado,
        "tag_tipo_usuario"=>$type_user
    );
