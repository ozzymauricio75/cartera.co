...
if(isset($fieldEditPost) && $fieldEditPost == "cobradordb"){
    ...
    if($fielvalid === true){
        ...
        $idAcreedorPost = (int)$_POST['datauser'];
        $idAcreedorPost = $db->escape($_POST['datauser']);
...
if(isset($fieldEditPost) && $fieldEditPost == "emailcobradordb"){
...
if(isset($fieldEditPost) && $fieldEditPost == "nicknamecobradordb"){
    ...
    if($fielvalid === true){
        ...
        //$db->where('id_usuario_plataforma', $idAcreedorPost);  
        $db->where('nickname_usuario', $fieldValue);
        $db->where('nickname_usuario', "", "!=");
        $goExist = $db->getOne('usuario_tbl', 'id_usuario, nickname_usuario, fecha_alta_usuario, tag_seccion_plataforma');
        
        if(count($goExist)>0){  
            $deudorid = $db->escape($goExist['id_usuario']);
            $deudornombre = $db->escape($goExist['nickname_usuario']);
            //$deudorapellido = $db->escape($goExist['primer_apellido_deudor']);
            $deudorcedula = $db->escape($goExist['tag_seccion_plataforma']);
            $deudorfecharegistro = date("d/m/Y", strtotime($goExist['fecha_alta_usuario']));
            
            
            $response = array(
                'existe' => 'true',
                'userid' => $deudorid,
                'username' => $deudornombre,
                //'deudorapellido' => $deudorapellido,
                'tipousuario' => $deudorcedula,
                'userfecharegistro' => $deudorfecharegistro,
            );
            //$response = $fieldRowEdit;                                    
        }else{            
            $response = array(
        ...
        foreach($recibeRules as $keyRules ){
            //foreach($valRules as $key => $v){
                ...
                switch($errFiel){
                    case 'fieldpost' :
                        $errValidaTmpl .= "<li class='list-group-item list-group-item-danger'><b>User name de usuario</b>
                        <br>".$usertyped." 
