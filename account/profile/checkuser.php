...
if(isset($fieldEditPost) && $fieldEditPost == "editaemail"){
    ...
    if($fielvalid === true){
        ...
        $db->where('mail_usuario', $fieldValue);
        $db->where('mail_usuario', "", "!=");
        $db->where('id_usuario_plataforma', $idPost, "!=");
        $goExist = $db->getOne('usuario_tbl', 'id_usuario_plataforma, nombre_usuario, nickname_usuario, tel_uno_usuario, tel_dos_usuario, cedula_usuario, ciudad_usuario, fecha_alta_usuario');
        
        if(count($goExist)>0){  
            $deudorid = $db->escape($goExist['id_usuario_plataforma']);
            $deudornombre = $db->escape($goExist['nombre_usuario']);
            //$deudorapellido = $db->escape($goExist['primer_apellido_deudor']);
            $deudorcedula = $db->escape($goExist['nombre_usuario']);
